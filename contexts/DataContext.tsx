...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Chicken Management
  const addChicken = (chickenData: Omit<Chicken, 'id'>) => {
    setChickens(prev => [...prev, { ...chickenData, id: Date.now().toString() }].sort((a,b) => a.name.localeCompare(b.name)));
  };
  const updateChicken = (updatedChicken: Chicken) => {
    setChickens(prev => prev.map(c => c.id === updatedChicken.id ? updatedChicken : c).sort((a,b) => a.name.localeCompare(b.name)));
  };
  ...
  const addTask = (taskData: Omit<Task, 'id' | 'isCompleted'>) => {
    setTasks(prev => [...prev, { ...taskData, id: Date.now().toString(), isCompleted: false }].sort((a,b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()));
  };
  const updateTask = (updatedTask: Task) => {
    setTasks(prev => prev.map(t => t.id === updatedTask.id ? updatedTask : t).sort((a,b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()));
  };
  const deleteTask = (id: string) => {
    setTasks(prev => prev.filter(t => t.id !== id));
  };
  const toggleTaskCompletion = (id: string) => {
    setTasks(prev => prev.map(t => t.id === id ? { ...t, isCompleted: !t.isCompleted } : t).sort((a,b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()));
  };

  // Food Log Management
  const addFoodLog = (foodLogData: Omit<FoodLog, 'id'>) => {
    setFoodLogs(prev => [...prev, { ...foodLogData, id: Date.now().toString() }].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
  };
  const updateFoodLog = (updatedFoodLog: FoodLog) => {
    setFoodLogs(prev => prev.map(fl => fl.id === updatedFoodLog.id ? updatedFoodLog : fl).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
  };
  ...
  const updateIncubationBatch = (updatedBatch: IncubationBatch) => {
    ...
    // Check if chicks are being recorded and haven't been recorded in inventory yet
    if (batchToUpdate.chicksHatched && batchToUpdate.chicksHatched > 0 && batchToUpdate.actualHatchDate && !batchToUpdate.hasBeenRecordedInInventory) {
      const newInventoryItem: Omit<InventoryItem, 'id' | 'currentQuantity'> = {
        ...
        birdType: "Pollito", // Specific type for newly hatched chicks
      ...
      // Call addInventoryItem directly, it handles setting currentQuantity
  ...
  const deleteIncubationBatch = (id: string) => {
    setIncubationBatches(prev => prev.filter(b => b.id !== id));
  };

  // Financial Transaction Management
  const addFinancialTransaction = (transactionData: Omit<FinancialTransaction, 'id'>) => {
    setFinancialTransactions(prev => [...prev, { ...transactionData, id: Date.now().toString() }].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
  };
  const updateFinancialTransaction = (updatedTransaction: FinancialTransaction) => {
    setFinancialTransactions(prev => prev.map(t => t.id === updatedTransaction.id ? updatedTransaction : t).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
  };
  ...
  const deleteInventoryItem = (id: string) => {
    ...
    setInventoryLogs(prevLogs => prevLogs.filter(log => log.inventoryItemId !== id)); // Cascade delete logs
  };
  ...
  const addInventoryLog = (logEntryData: Omit<InventoryLog, 'id'>) => {
    setInventoryItems(prevItems => 
      prevItems.map(item => {
        if (item.id === logEntryData.inventoryItemId) {
          ...
          // logEntryData.quantityChanged is always positive.
          // Action determines if it's an addition or subtraction.
          switch (logEntryData.action) {
            ...
            case InventoryAction.Adjustment:
              // For adjustments, quantityChanged could conceptually be positive or negative
              // but for simplicity, we assume form handles this and sends a positive
              // quantity with user choosing to "add" or "remove" via action or notes.
              // Here, we assume a positive quantity means increase, negative means decrease.
              // However, current type has quantityChanged as positive.
              // So, an "Adjustment" that reduces stock should be logged like a "Consumption"
              // or user must input a negative value if the form allowed.
              // To keep it simple: assume positive quantity for Addition, Sale, etc.
              // If an "Adjustment" needs to reduce, it should be logged as Spoilage/Consumption or
              // the form logic for "Adjustment" needs to be more complex.
              // For now, treat Adjustment as potentially increasing. If negative quantity is needed,
              // it means the input form should handle it. Assuming positive quantityChange here.
              // This part could be refined if "Adjustment" needs to handle negative changes directly.
              // Given current setup, if Adjustment means "decrease", it should act like Sale.
              // Let's assume an Adjustment log *increases* quantity if not specified otherwise.
              // This is a bit ambiguous. Let's make Adjustment decrease for now if not Addition.
              newCurrentQuantity -= logEntryData.quantityChanged; // Defaulting adjustment to decrease.
              break;
            default:
              break;
          }
          return { ...item, currentQuantity: Math.max(0, newCurrentQuantity) }; // Ensure not negative
        }
        return item;
      })
    );
    ...
    // If a sale log is added, also create a financial transaction
